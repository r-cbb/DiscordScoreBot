...
def scrapeESPN(delta=0):
    ...
    #MODE_ACTIVE = 0
    #MODE_INACTIVE = 1
    req = Request("http://www.espn.com/mens-college-basketball/scoreboard/_/group/50/?t=" + _returntime(delta))
...
def readlog():
    with open('discordscorebot/ncaagamelog.txt','r') as f:
        lines = f.readlines()
...
def gameinprocess(game,allgamelog):
    ...
    elif game['time'] == 'End of 2nd' and game['score1']==game['score2']:
        if not ifposted(allgamelog,game['tid1'],POS_ENDREG):
            return gamestring(game), updateprelog(allgamelog,game['tid1'],POS_ENDREG)
